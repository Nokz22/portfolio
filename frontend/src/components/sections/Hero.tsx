...
function ScrollIndicator() {
  return (
    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
      <span className="text-white/40 text-label-sm tracking-widest uppercase">scroll</span>
      <motion.div
        animate={{ y: [0, 8, 0] }}
...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="relative z-10 pt-20 pb-24 px-6 max-w-content mx-auto w-full">
        <motion.div
        ...
        >
          ...
          <motion.div variants={variants}>
            <span className="inline-flex items-center gap-2 bg-accent-subtle border border-accent/30 text-accent text-label-sm px-3 py-1 rounded-full">
          ...
          <motion.p
            variants={variants}
            className="font-display text-display-md text-ink-200"
          >
            {profile?.title ?? 'Junior Software Developer'}
          </motion.p>

          {/* Location */}
          <motion.p variants={variants} className="text-body-md text-ink-400 flex items-center gap-2">
            <svg
              xmlns="http://www.w3.org/2000/svg"
